...
describe("DynamoDBPostRepository", () => {
  ...
  describe("listPosts", () => {
    ...
    it("passes ExclusiveStartKey when cursor is provided", async () => {
      const cursor = Buffer.from(
        JSON.stringify({ pk: "POST#old", sk: "#METADATA" }),
      ).toString("base64");
      mockSend.mockResolvedValueOnce({
        Items: [],
        LastEvaluatedKey: undefined,
      ...
      const callArgs = vi.mocked(QueryCommand).mock.calls[0][0] as Record<
        string,
        unknown
      >;
      expect(callArgs.ExclusiveStartKey).toEqual({
        pk: "POST#old",
        sk: "#METADATA",
      });
    });
  ...
  describe("createPost", () => {
    it("creates a post with correct pk/sk and authorName derived from email", async () => {
      ...
      const putArgs = vi.mocked(PutCommand).mock.calls[0][0] as Record<
        string,
        unknown
      >;
