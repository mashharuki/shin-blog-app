...
vi.mock('../middleware/auth.js', () => ({
  cognitoAuthMiddleware: vi.fn(async (c: { set: (key: string, value: unknown) => void }, next: () => Promise<void>) => {
...
describe('GET /api/posts', () => {
  ...
  it('returns 200 with posts array when no cursor provided', async () => {
    mockListPosts.mockResolvedValueOnce({ posts: [samplePostSummary], nextCursor: undefined });

    const app = buildApp();
    const res = await app.request('/api/posts');

    expect(res.status).toBe(200);
    const body = await res.json() as { posts: PostSummary[]; nextCursor?: string };
  ...
  it('returns nextCursor in response when repository returns one', async () => {
    mockListPosts.mockResolvedValueOnce({ posts: [samplePostSummary], nextCursor: 'next-cursor-value' });
