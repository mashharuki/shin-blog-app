...
describe("NavBar", () => {
  ...
  describe("未認証状態 (Requirement 1.4)", () => {
    ...
    it("アバタードロップダウンが表示されない", () => {
      mockUseAuth.mockReturnValue({ user: null, isLoading: false, signIn: vi.fn(), signOut: vi.fn() });
      renderNavBar();
      expect(screen.queryByTestId("avatar-dropdown-trigger")).not.toBeInTheDocument();
    });
  });
  ...
  describe("認証済み状態 (Requirement 2.1, 2.2)", () => {
    ...
    it("「投稿する」ボタンが表示される", () => {
      mockUseAuth.mockReturnValue({ user: mockUser, isLoading: false, signIn: vi.fn(), signOut: vi.fn() });
      renderNavBar();
    ...
    it("ログアウトボタンをクリックすると signOut が呼ばれる (Requirement 2.2)", () => {
      ...
      mockUseAuth.mockReturnValue({ user: mockUser, isLoading: false, signIn: vi.fn(), signOut: mockSignOut });
      renderNavBar();
      fireEvent.click(screen.getByTestId("avatar-dropdown-trigger"));
